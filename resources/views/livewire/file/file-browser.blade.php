            ...
            <div class="flex items-center">
                @if ($this->query)
                    <div class="font-bold text-gray-400">
                        Showing you {{ $this->results->count() }} {{ Str::plural('result', $this->results->count()) }}. <button class="font-normal" wire:click="$set('query', null)">Clear search</button>
                    </div>
                @else
            ...
            <table class="w-full">
                ...
                <tbody>
                        ...
                        <tr class="border-gray-100 border-b-2 hover:bg-gray-100">
                            <td class="p-3">
                                <form class="flex items-center" wire:submit.prevent="createFolder">
                                    <input type="text" class="w-full px-3 h-10 border-2 @error('state.name') border-red-500 @else border-gray-200 @enderror rounded-lg mr-2"
                                    wire:model.defer="state.name">
                                    <button type="submit" class="bg-blue-600 text-white px-6 h-10 rounded-lg mr-2">
                                        Create
                                    </button>
                                    <button class="bg-gray-200 px-6 h-10 rounded-lg mr-2"
                    ...
                    @foreach ($this->results as $child)
                        <tr class="@if (!$loop->last) border-b-2 @endif border-gray-100 hover:bg-gray-100">
                            <td class="flex items-center py-2 px-3">
                                ...
                                @if ($renamingObject === $child->id)
                                    <form class="flex items-center ml-2 flex-grow" wire:submit.prevent="renameObject">
                                        <input type="text" class="w-full px-3 h-10 border-2 @error('renamingObjectState.name') border-red-500 @else border-gray-200 @enderror rounded-lg mr-2"
                                        wire:model.defer="renamingObjectState.name">
                                        <button type="submit" class="bg-blue-600 text-white px-6 h-10 rounded-lg mr-2">
                                            Rename
                                        </button>
                                        <button class="bg-gray-200 px-6 h-10 rounded-lg mr-2"
