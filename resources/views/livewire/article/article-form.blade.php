<x-jet-form-section submit="store"  class="w-full">
    ...
    <x-slot name="form">
        ...
        <div class="col-span-6 sm:col-span-2">
            ...
            <x-jet-input id="kicker" type="text" class="mt-1 block w-full" placeholder="Event Keyword" wire:model.defer="state.kicker" />
        ...
        <div class="col-span-6 sm:col-span-6">
            ...
            <x-jet-input id="title" type="text" class="mt-1 block w-full" placeholder="Enter Title" wire:model.defer="state.title"
            wire:change="titleChanged($event.target.value)" />
        ...
        <div class="col-span-6 sm:col-span-6">
            ...
            <x-jet-input id="slug" type="text" class="mt-1 block w-full" placeholder="Enter Slug" wire:model.defer="state.slug" />
                ...
                @foreach ($this->categories() as $category)
                    <div class="flex w-full border border-gray-300 mb-1 p-1 rounded-md">
                        <div class="mr-2 text-lg">
                            ...
                            <input type="checkbox" class="form-checkbox" value="{{ $category->id }}" wire:model="state.categories.{{ $category->id }}">
                        ...
                        @if($category->children->count())
                            <div class="flex flex-wrap items-center">
                                @foreach ($category->children as $child)
                                    <div class="mr-2 text-lg">
                                        <label class="flex items-center">
                                        <input type="checkbox" class="form-checkbox" value="{{ $child->id }}" wire:model="state.categories.{{ $child->id }}">
                                    ...
                                    @if($child->children->count())
                                        @foreach ($child->children as $subChild)
                                            <div class="mr-2 text-lg">
                                                <label class="flex items-center">
                                                <input type="checkbox" class="form-checkbox" value="{{ $subChild->id }}" wire:model="state.categories.{{ $subChild->id }}">
                                                    <span class="ml-1 text-lg text-gray-600">{{ $subChild->name }}</span>
                                                </label>
                                            </div>
                                        @endforeach
                                    @endif
        ...
        <div class="col-span-6 sm:col-span-6">
            ...
            <div class="flex flex-wrap items-center mt-1">
                @foreach ($this->topics() as $topic)
                    <div class="mr-2 text-lg">
                        <label class="flex items-center">
                        <input type="checkbox" class="form-checkbox" value="{{ $topic->id }}" wire:model="state.topics.{{ $topic->id }}">
